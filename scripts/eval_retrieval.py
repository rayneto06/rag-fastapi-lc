import os, csv, json, math, textwrap, datetime
...
# Split & retrieval params (shared by all models)
CHUNK_SIZE = 1000
CHUNK_OVERLAP = 150
TOP_K = 5

# Diagnostics: print top-K candidate chunks per question (per model)
EVAL_DUMP = False            # set to False for a clean metrics-only run
EVAL_DUMP_K = 10            # how many candidates per question/model
PREVIEW_CHARS = 220

# Gold set with (question, relevant_chunk_ids ; separated)
GOLD_PATH = "data/eval/eval_gold.csv"

# Which models to evaluate (comment out any you don't want)
# type: "ollama" or "hf"
MODELS = [
    # Your current generic baseline (multilingual-ish)
    {"tag": "generic_ollama", "type": "ollama", "model": "nomic-embed-text", "base_url": "http://localhost:11434"},
    # PubMed (English scientific STS/NLI) — useful for English biomedical data
    {"tag": "pubmed_sbiomed", "type": "hf", "model": "pritamdeka/S-Biomed-Roberta-snli-multinli-stsb"},
    # Strong multilingual retrieval models to try with PT-BR corpus:
    {"tag": "bge_m3", "type": "hf", "model": "BAAI/bge-m3"},
    {"tag": "mpnet_multi", "type": "hf", "model": "sentence-transformers/paraphrase-multilingual-mpnet-base-v2"},
]
...
# LangChain (non-deprecated packages)
...
def retrieve_ids(vs: Chroma, question: str, k: int) -> List[str]:
    return [(d.metadata or {}).get("chunk_id", "unknown#c?") for d in retrieve_docs(vs, question, k)]
...
def build_embeddings(entry: Dict) -> object:
    ...
    if kind == "ollama":
        ...
        # If you need a gated model: set HUGGINGFACE_HUB_TOKEN in your shell (not needed for public repos)
...
def write_metrics_csv(out_path: str, rows: List[Dict[str, str]]) -> None:
    ...
    fieldnames = ["tag", "model_id", "recall_at_k", "mrr_at_k", "ndcg_at_k", "k", "chunk_size", "chunk_overlap"]
...
def print_summary_table(rows: List[Dict[str, str]]) -> None:
    ...
    print(f"{'Tag':<18} {'Model':<48} {'Recall@'+str(TOP_K):>10} {'MRR@'+str(TOP_K):>10} {'NDCG@'+str(TOP_K):>10}")
    for r in rows:
        print(f"{r['tag']:<18} {r['model_id']:<48} {float(r['recall_at_k']):>10.3f} {float(r['mrr_at_k']):>10.3f} {float(r['ndcg_at_k']):>10.3f}")
    print("="*86)
...
def main():
    ...
    for entry in selected:
        ...
        # Optional: diagnostic dump per model
        if EVAL_DUMP:
            for case in gold:
